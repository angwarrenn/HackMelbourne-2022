...
function GetEvent({ email }) {
  ...
  const url = process.env.REACT_APP_SERVER + "/retrieve-events";
  ...
  const genIcal = () => {
    ...
    const url = process.env.REACT_APP_SERVER + "/gen-ics";
    const uuid = uuidv4();
    ...
    window.open(process.env.REACT_APP_SERVER + "/ics/" + uuid + ".ics");
  };
  ...
  function printCard(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
      elements.push(
        <EventCard date={arr[i][0]} name={arr[i][1]} desc={arr[i][2]} />
      );
    }
    return elements;
  }
  ...
  return (
    <div>
      <h3>Events:</h3>
      <br></br>
      <button className="download" onClick={genIcal}>
        Download events <IoCloudDownload />
      </button>
      <br></br>
